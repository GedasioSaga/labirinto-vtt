...
import { mapaDoPiso, pisoDe, temPisos } from './pisos'
...
export function imposedMovement(map: MapData, radii: OwnerVisionRadii = NO_OWNER_RADII): ImposedMovement {
  if (!temPisos(map)) return imposedMovementOnFloor(map, radii)
  // PISOS NA MESMA CENA: a sala da esteira, a parede que segura, o pino da
  // cabine e quem ocupa a casa são do piso da ficha (`mapaDoPiso`). A ficha do
  // térreo sob a esteira do mezanino não anda, e a do outro piso não segura.
  const changed = new Map<string, Token>()
  const transfers: CabinTransfer[] = []
  for (const piso of new Set(map.tokens.map(pisoDe))) {
    const floor = mapaDoPiso(map, piso)
    const next = imposedMovementOnFloor(floor, radii)
    transfers.push(...next.transfers)
    if (next.map === floor) continue
    for (const token of next.map.tokens) changed.set(token.id, token)
  }
  if (changed.size === 0) return { map, transfers }
  return { map: { ...map, tokens: map.tokens.map((token) => changed.get(token.id) ?? token) }, transfers }
}

/** Um Avançar num piso só: `map` é o recorte de um piso (`mapaDoPiso`) ou o mapa de um piso só. */
function imposedMovementOnFloor(map: MapData, radii: OwnerVisionRadii): ImposedMovement {
