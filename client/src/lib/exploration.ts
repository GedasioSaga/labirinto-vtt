...
/**
 * PASSAR O MAPA, TELA DA MESA e MEMÓRIA POR FICHA (o jogador herda o que a
 * ficha dele viu): soma em `into` o que `from` explorou (células
...
export function mergeExploration(into: Exploration, from: Exploration, concealed: readonly RegionPoint[][] = []): boolean {
  ...
  for (const ring of from.rings) {
    ...
    // Cópia: a memória da ficha e a do jogador não podem dividir o mesmo anel.
    into.rings.push({ ...ring, points: ring.points.map((p) => ({ x: p.x, y: p.y })) })
