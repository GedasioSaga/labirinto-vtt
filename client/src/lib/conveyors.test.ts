...
describe('advanceConveyors — "Fichas ocupam espaço" vale para a esteira', () => {
  ...
  it('ficha vista pela borda (o centro atrás da parede, o corpo na casa) segura: o jogador a recebe pela borda', () => {
    // O ogro grande (3 casas) está na sala B; a borda dele invade a casa (475, 75) da sala A.
    // O recorte manda a ficha que tem parte do corpo na visão (`ficha-vista-pela-borda`):
    // Ana o vê de (425, 75), então parar antes dele não conta nada que o jogador não saiba.
    const antes = ocupada([ficha('ana', 325, 75), ficha('ogro', 560, 75, { size: 3 })])
    const depois = advanceConveyors(antes)
    expect(posicao(depois, 'ana')).toEqual({ x: 425, y: 75 })
    expect(posicao(depois, 'ogro')).toEqual({ x: 560, y: 75 })
    const deOndeParou = filterMapForGroup({ ...antes, tokens: depois.tokens }, [{ tokenIds: ['ana'], visionRadius: 700 }])
    expect(deOndeParou.map.tokens.map((t) => t.id)).toContain('ogro')
  })
  ...
  it('o dragão além do raio de visão do dono de Ana não segura a esteira; dentro do raio, segura', () => {
    // O dragão (6 casas) está na sala B, com a porta A|B aberta: o corpo dele toma as casas
    // (475, 200) e (525, 200). Com um raio de 10 px o jogador de Ana não o recebe de casa
    // nenhuma do caminho, e a esteira a larga na terceira casa, sobre o chão que ele vê vazio.
    const mapa = ocupada([ficha('ana', 375, 200), ficha('dragao', 625, 200, { size: 6 })])
    const raioCurto = advanceConveyors(mapa, new Map([['ana', 10]]))
    expect(posicao(raioCurto, 'ana')).toEqual({ x: 525, y: 200 })
    expect(posicao(raioCurto, 'dragao')).toEqual({ x: 625, y: 200 })
    // Com 50 px, o corpo só entra na visão quando Ana encosta nele, em (475, 200): segura ali.
    expect(posicao(advanceConveyors(mapa, new Map([['ana', 50]])), 'ana')).toEqual({ x: 475, y: 200 })
    // Raio que o alcança de longe: à vista, segura na casa antes, como no movimento manual.
