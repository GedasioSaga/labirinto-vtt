 ...
 * (`Pin.cabineContinua`); a cada "Avançar esteiras", a ficha que ficou parada na casa
 ...
 * paternoster que muda de andar. `Pin.cabineContinua` guarda o id do par. Aqui só se
...
/**
 * Parada DESTA cena: pino "!"/"?". O de viagem leva ao par dele, nunca a outro
 * pino da cena; a alavanca é um mecanismo de porta, não parada.
 */
function canBeCabin(pin: Pin): boolean {
  return pin.kind === 'exclamacao' || pin.kind === 'interrogacao'
}
...
function parStop(pin: Pin): PinDestination | null {
  if (pin.cabineContinua === undefined) return null
  return parDestinations(pin).find((destino) => destino.pinId === pin.cabineContinua) ?? null
}

/** `Pin.cabineContinua` como veio do disco (cru): só texto não vazio; o resto é ausência. */
...
export function pinWithCabin(pin: Pin, cabine: string | undefined): Pin {
  const { cabineContinua: _antiga, ...semCabine } = pin
  return cabine === undefined ? semCabine : { ...semCabine, cabineContinua: cabine }
}

/** A próxima parada que vale: pino desta cena, outro, e que pode ser cabine. */
function nextStop(map: MapData, pin: Pin): Pin | null {
  if (!canBeCabin(pin) || pin.cabineContinua === undefined || pin.cabineContinua === pin.id) return null
  const target = map.pins.find((p) => p.id === pin.cabineContinua)
...
export function setPinCabin(map: MapData, pinId: string, targetId: string | null): MapData {
  ...
  if (targetId === null) {
    return pin.cabineContinua === undefined ? map : { ...map, pins: map.pins.map((p) => (p === pin ? pinWithCabin(p, undefined) : p)) }
  }
  if (targetId === pinId || targetId === pin.cabineContinua) return map
  const valid = viagem
