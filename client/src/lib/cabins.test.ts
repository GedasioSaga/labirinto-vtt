...
function elevador(tokens: Token[], extra: Partial<MapData> = {}): MapData {
  return {
    ...torre({ tokens }),
    pins: [
      pino('parada-a1', 75, 75, { cabineContinua: 'parada-a2', description: 'Cabine do térreo' }),
      pino('parada-a2', 375, 325, { cabineContinua: 'parada-c1' }),
      pino('parada-c1', 1275, 75, { cabineContinua: 'parada-a1' }),
      pino('cartaz', 225, 225),
    ],
    ...extra,
  }
...
describe('advanceConveyors — a cabine contínua leva quem ficou parado no pino', () => {
  ...
  it('pino sem cabine, cabine que aponta para pino apagado e pino de viagem não levam ninguém', () => {
    ...
    const orfa: MapData = { ...torre({ tokens: [ficha('ana', 75, 75)] }), pins: [pino('p1', 75, 75, { cabineContinua: 'sumiu' })] }
    expect(advanceConveyors(orfa)).toBe(orfa)
    const viagem: MapData = {
      ...torre({ tokens: [ficha('ana', 75, 75)] }),
      pins: [pino('p1', 75, 75, { kind: 'viagem', cabineContinua: 'p2' }), pino('p2', 375, 325)],
    }
  ...
  it('quem a esteira moveu neste Avançar não pega a cabine; quem a esteira não conseguiu mover pega', () => {
    ...
    const parada: MapData = { ...elevador([ficha('ana', 75, 25)], esteiraNorte), pins: [pino('p1', 75, 25, { cabineContinua: 'p2' }), pino('p2', 375, 325)] }
  ...
  it('com "Fichas ocupam espaço", cabine cheia no destino segura a ficha; troca entre duas paradas passa', () => {
    ...
    const terminal = (extra: Partial<MapData>): MapData => ({
      ...
      pins: [pino('p1', 75, 75, { cabineContinua: 'p2' }), pino('p2', 375, 325)],
      ...extra,
    })
    ...
    const dupla: MapData = {
      ...
      pins: [pino('p1', 75, 75, { cabineContinua: 'p3' }), pino('p2', 375, 75, { cabineContinua: 'p3' }), pino('p3', 225, 325)],
  ...
  it('com "Fichas ocupam espaço", só segura a cabine quem Ana enxerga do pino onde está', () => {
    ...
    const longe: MapData = {
      ...
      pins: [pino('p1', 75, 75, { cabineContinua: 'p2' }), pino('p2', 1275, 75)],
      ...ocupa,
    }
  ...
  it('com "Fichas ocupam espaço", o guarda além do raio de visão do dono de Ana não segura a cabine', () => {
    ...
    const mapa: MapData = {
      ...
      pins: [pino('p1', 125, 200, { cabineContinua: 'p2' }), pino('p2', 975, 200)],
      movement: { tokensOccupy: true },
    }
...
describe('setPinCabin — o mestre liga o pino ao próximo', () => {
  ...
  it('o mesmo destino, o próprio pino, pino que não existe e pino de viagem devolvem o MESMO mapa', () => {
    const base: MapData = {
      ...torre(),
      pins: [pino('p1', 75, 75, { cabineContinua: 'p2' }), pino('p2', 375, 325), pino('v1', 225, 225, { kind: 'viagem' })],
    }
...
describe('cabine no arquivo', () => {
  it('volta igual do arquivo; mapa antigo abre sem o campo; lixo sai', () => {
    const map = elevador([])
    expect(deserializeMap(serializeMap(map)).pins.map((p) => p.cabineContinua)).toEqual(['parada-a2', 'parada-c1', 'parada-a1', undefined])
    expect('cabineContinua' in (deserializeMap(serializeMap({ ...torre(), pins: [pino('p1', 1, 1)] })).pins[0] ?? {})).toBe(false)
    expect(readCabin(42)).toBeUndefined()
    expect(readCabin('')).toBeUndefined()
    expect(readCabin('p2')).toBe('p2')
    expect(deserializeMap('{"id": "x", "pins": [{"id": "p1", "x": 1, "y": 1, "cabineContinua": {"id": 3}}]}').pins[0]?.cabineContinua).toBeUndefined()
  })
})
