...
describe('fogFilter: passagem só de ida', () => {
  it('pino de uma saída com o par só de chegada sai com semVolta, e nada do destino', () => {
    const view = filterMapForPlayer(mapaCom([CALHA]), 'p1', POSSE, RAIO, undefined, undefined, undefined, undefined, soIda([['calha', [SAIDA_PRINCIPAL]]]))
  ...
  it('marca gravada no pino do mestre (arquivo à mão) não atravessa: quem decide é o host', () => {
    ...
    const view = filterMapForPlayer(mapaCom([forjado]), 'p1', POSSE, RAIO, undefined, undefined, undefined, undefined, soIda([]))
    expect(view.map.pins.map((p) => p.id)).toEqual(['calha'])
    expect(view.map.pins[0].semVolta).toBeUndefined()
  })

  it('encruzilhada: só a saída cujo par é chegada oculta ganha soIda; o pino não ganha semVolta', () => {
    const view = filterMapForPlayer(mapaCom([CRUZ]), 'p1', POSSE, RAIO, undefined, undefined, undefined, undefined, soIda([['cruz', ['saida_poco']]]))
  ...
  it('pino fora da visão não sai — nem a marca de só ida dele', () => {
    ...
    const view = filterMapForPlayer(mapaCom([longe]), 'p1', POSSE, RAIO, undefined, undefined, undefined, undefined, soIda([['calha', [SAIDA_PRINCIPAL]]]))
