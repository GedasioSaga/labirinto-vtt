...
describe('cabine do pino de viagem — liga ao par', () => {
  ...
  it('volta igual do arquivo', () => {
    ...
    expect(deserializeMap(serializeMap(map)).pins[0]?.cabine).toBe('poco-sotao')
...
describe('imposedMovement — quem ficou parado no pino de viagem vai ao par', () => {
  it('a ficha parada no poço sai para o par, e o mapa desta cena fica igual (quem troca de cena é a aventura)', () => {
    const map = terreo([poco({ cabine: 'poco-porao' })])
  ...
  it('pela segunda saída, ao par dela', () => {
    expect(imposedMovement(terreo([poco({ cabine: 'poco-sotao' })])).transfers).toEqual([{ tokenId: 'ana', sceneId: 'sotao', pinId: 'poco-sotao' }])
  })
  ...
  it('ninguém vai: poço sem cabine, ficha fora da casa do pino, cabine para par que não é de nenhuma saída, chegada oculta', () => {
    ...
    expect(imposedMovement(terreo([poco({ cabine: 'poco-porao' })], [ficha('ana', 125, 75)])).transfers).toEqual([])
    expect(imposedMovement(terreo([poco({ cabine: 'sumiu' })])).transfers).toEqual([])
    const oculta = terreo([poco({ cabine: 'poco-porao', soChegada: true })])
  ...
  it('quem a esteira moveu neste Avançar estava andando: não pega a cabine; quem ficou parado pega', () => {
    ...
    const andando = imposedMovement(terreo([poco({ cabine: 'poco-porao' })], [ficha('ana', 75, 75), ficha('bia', 375, 75)], esteiraSul))
    ...
    const parada = imposedMovement(terreo([poco({ x: 75, y: 25, cabine: 'poco-porao' })], [ficha('ana', 75, 25), ficha('bia', 375, 325)], esteiraNorte))
