...
describe('tela da mesa: só olha', () => {
  ...
  it('pino "Só estes" (quem vê) não sai na tela: a TV é vista por todos na mesa', () => {
    const { s, ana } = mesaMontada()
    s.handleMessage('c-tv', ENTRAR_COMO_MESA, mundo())
    s.setTableScene('s-salao')
    // Controle: sem lista, o altar que Ana vê sai na tela.
    expect(snapshotDaTela(s.broadcast(mundo())).map.pins.map((p) => p.id)).toEqual(['altar'])
    s.setPinAudience('altar', [ana])
    const r = s.broadcast(mundo())
    expect(snapshotDaTela(r).map.pins).toEqual([])
    expect(JSON.stringify(paraTela(r))).not.toContain('Altar de pedra')
  })
