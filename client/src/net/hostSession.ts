import type { AreaTriggerKind, DoorState, HazardKind, MapData, Pin, PinPassage, RegionPoint, Token, TokenContract, Wall } from '../types/map'
...
function restoredMemoryOf(scene: SavedSceneMemory): Omit<PlayerMemory, 'place'> | null {
  ...
  return { key: memoryKey(dims), dims, exp, doors, doorSeen: new Map(), vision: [], restored: true, seenRooms: new Set() }
}
...
export function createHostSession(options: HostSessionOptions): HostSession {
  ...
  const memoryFor = (playerId: string, map: MapData, world: HostWorld): PlayerMemory => {
    ...
    const memory: PlayerMemory = existingMemory(playerId, map) ?? {
  ...
  const viewFor = (playerId: string, world: HostWorld, arrived: 'on_change' | 'always'): HostMessage[] => {
    ...
    const view = snapshotFor(playerId, scene, world, sceneNameForPlayer(scene), floorsFor(playerId, scene, world))
   ...
   */
  const snapshotFor = (playerId: string, scene: HostScene, world: HostWorld, sceneName: string | undefined, andares: FloorsWire | undefined): HostMessage[] => {
    const map = scene.map
   ...
   */
  const doorSeenBy = (playerId: string, map: MapData, wallId: string, world: HostWorld): { wall: Wall; door: DoorState; near: boolean; key: string | null } | null => {
  ...
  function handleTokenEdit(clientId: string, msg: TokenEditMessage, world: HostWorld): HostResult {
    ...
    if (helperLoans.has(msg.tokenId) || loans.get(msg.tokenId)?.borrowerId === playerId) return { outbound: [] }
  ...
  const api: HostSession = {
    ...
    revealPlan(playerId, source) {
      ...
      if (scene === null) return
      const map = scene.map
      markAll(memoryFor(playerId, map, world).exp, playerBlockedRings(floorMapOf(playerId, map)))
    },
    ...
    broadcast(source) {
      ...
      for (const [clientId, playerId] of byClient) {
        ...
        // e o recado só para ele guardado também — quem ganhou ficha agora o lê.
        outbound.push(...viewWithPendingNote(clientId, playerId, viewFor(playerId, world, 'on_change')))
      ...
      for (const ownerId of new Set([...loans.values()].map((loan) => loan.ownerId))) {
        ...
        if (scene !== null) snapshotFor(ownerId, scene, world, sceneNameForPlayer(scene), floorsFor(ownerId, scene, world))
      }
