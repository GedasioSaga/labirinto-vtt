...
export function createHostSession(options: HostSessionOptions): HostSession {
   ...
   * não está em cena nenhuma) e, logo atrás, o último recado da cena quando
   * isto é uma CHEGADA a ela. `arrived: 'always'` é a entrada/volta à sala:
   * o recado vem mesmo que ele já o tenha (o cartão reaparece).
   */
  ...
  function handleJoin(clientId: string, msg: JoinMessage, world: HostWorld): HostResult {
    ...
    const [next, ...note] = statusOf(record.playerId) === 'playing' ? viewFor(record.playerId, world, 'always') : waiting
    ...
    for (const msg of note) outbound.push({ clientId, msg })
    return { outbound }
  }
