...
const RAIO_TAVERNA = 300
const CENA_TAVERNA = 'cena-taverna'
...
const mundoTaverna = (anaX: number, anaY: number): HostWorld => ({ open: { sceneId: CENA_TAVERNA, name: 'Taverna', map: taverna(anaX, anaY) }, background: [] })

function playerIdDo(result: HostResult): string {
...
function mesaTaverna() {
  let n = 0
  const s = createHostSession({
    code: CODE,
    visionRadius: RAIO_TAVERNA,
    now: () => 1_000_000,
    randomId: () => {
      n += 1
      return `id-${n}`
  ...
  const ana = playerIdDo(s.handleMessage('c-ana', { type: 'join', code: CODE, name: 'Ana' }, mundoTaverna(200, 200)))
...
describe('hostSession: pino só de perto e pino marco', () => {
  it('Ana na porta: a carta chega marcada longe e o pacote NÃO leva o texto', () => {
    const { s } = mesaTaverna()
    const r = s.broadcast(mundoTaverna(200, 200))
  ...
  it('Ana anda até o lado da mesa: o próximo pacote já leva o texto; ao se afastar, o texto sai de novo', () => {
    const { s } = mesaTaverna()
    expect(pacoteDa(s.broadcast(mundoTaverna(200, 200)))).not.toContain(TEXTO_DA_CARTA)
    const aoLado = s.broadcast(mundoTaverna(350, 200))
    expect(pinosDa(aoLado).find((p) => p.id === 'carta')?.description).toBe(TEXTO_DA_CARTA)
    const deVolta = s.broadcast(mundoTaverna(200, 200))
  ...
  it('"Revelar planta" não entrega as pistas da cena: a carta e o diário aparecem sem texto', () => {
    const { s, ana } = mesaTaverna()
    const w = mundoTaverna(200, 200)
    s.broadcast(w)
    s.revealPlan(ana, w)
  ...
  it('Templo marco: chega à Ana, que nunca foi lá, com o texto', () => {
    const { s } = mesaTaverna()
    const templo = pinosDa(s.broadcast(mundoTaverna(200, 200))).find((p) => p.id === 'templo')
