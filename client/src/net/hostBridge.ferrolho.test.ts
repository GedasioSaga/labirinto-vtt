...
async function mesa(portaAberta = false) {
  let porta: DoorState = { open: portaAberta, locked: false, kind: 'normal' }
  const mapa = (): MapData => ({
  ...
  // Cada mensagem anda 1 s no relógio: o limite de um pedido de porta por 250 ms
  // não segura o passo seguinte, e a sala não acha que alguém caiu (o relógio
  // só anda entre mensagens, não a cada leitura).
  let t = 0
  const bridge = createHostBridge({ invoke, listen, getMap: () => mapa(), getWorld: world, applyMove: vi.fn(), applyDoor, onPlayersChange: vi.fn(), now: () => t })
...
describe('hostBridge: ferrolho do jogador', () => {
  ...
  it('Ana passa o ferrolho numa porta aberta: a porta fecha e o mestre lê UM aviso, o do ferrolho', async () => {
    const { emit, applyDoor } = await mesa(true)
    emit('net:message', { clientId: 'c1', msg: { type: 'door.bar', wallId: 'porta', on: true } })
    expect(applyDoor).toHaveBeenCalledWith('porta', false)
    const textos = useToastStore.getState().toasts.map((t) => t.text)
    expect(textos).toContain('Ana passou o ferrolho numa porta')
    // "Ana fechou a porta" junto seria o mesmo gesto contado duas vezes.
    expect(textos).not.toContain('Ana fechou a porta')
  })
