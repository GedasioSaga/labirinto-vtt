...
/** A Prisão com uma Sala "Cela" (texto ao entrar) em volta de onde as fichas ficam (100, 100). */
function prisaoComCela(world: HostWorld, notaDoMestre?: string): HostWorld {
  const cela: Region = {
    id: 'cela',
    points: [
      { x: 50, y: 50 },
    ...
    fillPattern: 'solid',
    data: {},
    room: { shape: 'rect', name: 'Cela', textoAoEntrar: 'Palha úmida no chão.', ...(notaDoMestre === undefined ? {} : { notaDoMestre }) },
  }
  return { ...world, background: world.background.map((c) => (c.sceneId === 's-prisao' ? { ...c, map: { ...c.map, regions: [cela] } } : c)) }
}
...
describe('caderno de recados (host)', () => {
  ...
  it('Gabi chega à Prisão dentro de uma Sala com texto: mapa, cartão da Sala e recado, nessa ordem; a nota do mestre e o recado não vão a Diego', () => {
    const NOTA = 'SEGREDO-DO-MESTRE: o carcereiro é o traidor'
    const comCela = (gabiNaPrisao: boolean): HostWorld => prisaoComCela(mundo(gabiNaPrisao), NOTA)
    const { s } = mesa()
    s.sceneNote('s-prisao', RECADO, comCela(false))
  ...
  it('Gabi cai e volta na Cela: welcome, mapa, caderno e recado; o cartão da Sala não repete', () => {
    const w = prisaoComCela(mundo(true))
    const { s, gabi } = mesa()
    s.broadcast(w)
