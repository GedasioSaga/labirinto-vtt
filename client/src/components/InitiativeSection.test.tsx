...
describe('InitiativeSection (aba Jogo)', () => {
  ...
  it('cada entrada da ordem separa o nome do valor no próprio texto ("Machado 17", nunca "Machado17")', () => {
    render({ values: { lanterna: 12, machado: 17, goblin: 5 } })
    const lista = secao().querySelector('[aria-label="Ordem de iniciativa"]')
    // textContent é o que a régua lê (toContainText); sem separador os dois spans colam.
    const itens = Array.from(lista?.querySelectorAll('li') ?? []).map((li) => (li.textContent ?? '').trim())
    expect(itens).toEqual(['Machado 17', 'Lanterna 12', 'Goblin 5'])
    for (const texto of itens) expect(texto).toMatch(/^\p{L}+\s+\d+$/u)
  })
