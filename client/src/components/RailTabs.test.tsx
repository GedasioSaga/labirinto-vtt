...
describe('RailTabs', () => {
  ...
  it('começa em Mapa com o painel da sala escondido', () => {
    ...
    expect(tab('Jogo').getAttribute('aria-selected')).toBe('false')
    // A aba de multiplayer não pode mais se chamar "Sala" (nome da ferramenta).
    expect(() => tab('Sala')).toThrow('sem aba Sala')
    expect(panelOf(tab('Mapa')).hidden).toBe(false)
    expect(panelOf(tab('Jogo')).hidden).toBe(true)
    expect(panelOf(tab('Jogo')).getAttribute('aria-labelledby')).toBe(tab('Jogo').id)
  })
  ...
  it('clique troca o painel e o aria-selected', () => {
    ...
    expect(panelOf(tab('Jogo')).textContent).toBe('painel sala')
  ...
  it('setas trocam de aba e movem o foco', () => {
    ...
    press('ArrowLeft')
    expect(tab('Jogo').getAttribute('aria-selected')).toBe('true')
    expect(tab('Jogo').tabIndex).toBe(0)
