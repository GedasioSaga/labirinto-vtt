...
describe('Lugar ocupado no cliente do jogador', () => {
  ...
  it('outra recusa (parede) volta a ficha sem aviso de lugar ocupado: o aviso é o da parede', () => {
    const { connection, socket } = jogando()
    connection.requestMove('heroi', 225, 125)
    socket.receive({ type: 'token.move.rejected', reqId: lastReqId(socket), reason: 'wall' })
    // Junção com o aviso de recusa do movimento (grupo jogador): cada motivo
    // tem a sua frase ("Parede no caminho"); 'occupied' não sai de parede.
