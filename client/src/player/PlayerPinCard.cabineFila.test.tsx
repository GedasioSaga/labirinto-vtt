...
describe('PlayerPinCard: chamar a cabine', () => {
  ...
  it('cabine longe: oferece "Chamar a cabine"; tocar chama uma vez e o cartão diz que o chamado foi', () => {
    ...
    act(() => root.render(<PlayerPinCard pin={parada({ cabine: 'longe' })} onClose={vi.fn()} onRequestTravel={vi.fn()} onChamarCabine={chamar} />))
  ...
  it('o chamado não saiu (sem conexão): o botão continua lá para tentar de novo', () => {
    ...
    act(() => root.render(<PlayerPinCard pin={parada({ cabine: 'longe' })} onClose={vi.fn()} onChamarCabine={chamar} />))
    act(() => botao('Chamar a cabine').click())
    expect(chamar).toHaveBeenCalledTimes(1)
    expect(botoes()).toEqual(['Chamar a cabine', 'Fechar'])
  })
  ...
  it('parada já chamada: diz que a cabine vem, sem botão de passar nem de chamar de novo', () => {
    act(() => root.render(<PlayerPinCard pin={parada({ cabine: 'chamada' })} onClose={vi.fn()} onRequestTravel={vi.fn()} onChamarCabine={vi.fn(() => true)} />))
  ...
  it('cabine aqui com alguém dentro: diz que está ocupada, sem botão de passar nem de chamar', () => {
    act(() => root.render(<PlayerPinCard pin={parada({ cabine: 'ocupada' })} onClose={vi.fn()} onRequestTravel={vi.fn()} onChamarCabine={vi.fn(() => true)} />))
  ...
  it('trancada ganha de tudo: nada de chamar a cabine', () => {
    act(() => root.render(<PlayerPinCard pin={parada({ cabine: 'longe', passagem: 'trancada' })} onClose={vi.fn()} onChamarCabine={vi.fn(() => true)} />))
