...
describe('PlayerPanel: aba Caderno', () => {
  ...
  /** jsdom não tem matchMedia: o stub faz o papel da janela (larga ou de celular). Mesmo corte de player.css. */
  ...
  it('no notebook com o painel recolhido, recado novo NÃO conta como lido; abrir o painel no Caderno conta', () => {
    janela('notebook')
    const onRead = vi.fn()
    render([], false, onRead)
    act(() => aba(/Caderno/).click())
    act(() => botaoPainel().click())
    expect(botaoPainel().getAttribute('aria-expanded')).toBe('false')
    render([{ id: 'n1', text: 'oi', at: AS_20_30 }], true, onRead)
    expect(onRead).not.toHaveBeenCalled()
    // O ponto fica no "Painel", que é o que está à vista.
  ...
  it('no celular com a gaveta fechada, o Caderno não conta como lido até a gaveta abrir', () => {
    janela('celular')
    const onRead = vi.fn()
    render([{ id: 'n1', text: 'oi', at: AS_20_30 }], true, onRead)
    act(() => botaoPainel().click())
    act(() => aba(/Caderno/).click())
    expect(onRead).toHaveBeenCalledTimes(1)
    // Fecha a gaveta (Escape é dela) e chega outro recado: fica novo.
    act(() => {
      document.body.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true }))
    })
    expect(botaoPainel().getAttribute('aria-expanded')).toBe('false')
    render([{ id: 'n1', text: 'oi', at: AS_20_30 }, { id: 'n2', text: 'outro', at: AS_21_05 }], true, onRead)
    expect(onRead).toHaveBeenCalledTimes(1)
    expect(botaoPainel().querySelector('.pp-unread')).not.toBeNull()
  })

  it('o nome do botão Painel com recado novo diz que há recado novo', () => {
    janela('celular')
    render([{ id: 'n1', text: 'oi', at: AS_20_30 }], true)
    expect(botaoPainel().getAttribute('aria-label')).toBeNull()
    expect(botaoPainel().textContent).toBe('Painel (recado novo)')
  })
