 ...
 *
 * `pixel` (opcional): traço com `settings.lineWidth` px de TELA, cada linha no
 * pixel físico inteiro (pixelAlign.ts). Sem ele a espessura é em px de mundo.
 */
export function drawGrid(graphics: Graphics, lines: GridLine[], viewport: Viewport, settings: GridSettings, pixel?: PixelGrid): void {
  graphics.clear()
  if (lines.length === 0) return

  for (const line of lines) {
    const position = pixel ? alignToPixel(line.position, pixel) : line.position
    if (line.axis === 'x') {
      strokeDashedSegment(graphics, position, viewport.top, position, viewport.bottom, settings.lineStyle)
    } else {
      strokeDashedSegment(graphics, viewport.left, position, viewport.right, position, settings.lineStyle)
    }
  }

  graphics.stroke({
    width: pixel ? strokeWidthInWorld(pixel) : settings.lineWidth,
